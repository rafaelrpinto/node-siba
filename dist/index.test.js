...
test("should generate a valid SIBA envelope", () => {
    const { Envelope } = _1.buildSIBASoapEnvelope(bulletin);
    ...
    expect(Boletim_Alojamento.Data_Entrada.value).toBe(bulletin.guest.checkInDate.toISOString());
    expect(Boletim_Alojamento.Data_Nascimento.value).toBe(bulletin.guest.birthDate.toISOString());
    expect(Boletim_Alojamento.Data_Saida.value).toBe(bulletin.guest.checkOutDate && bulletin.guest.checkOutDate.toISOString());
    expect(Boletim_Alojamento.Documento_Identificacao.value).toBe(bulletin.guest.document.number);
    expect(Boletim_Alojamento.Local_Nascimento.value).toBe(bulletin.guest.birthPlace);
    expect(Boletim_Alojamento.Local_Residencia_Origem.value).toBe(bulletin.guest.placeOfResidence);
    expect(Boletim_Alojamento.Nacionalidade.value).toBe(bulletin.guest.nationality);
    expect(Boletim_Alojamento.Nome.value).toBe(bulletin.guest.surname);
    expect(Boletim_Alojamento.Pais_Emissor_Documento.value).toBe(bulletin.guest.document.issuingCountry);
    expect(Boletim_Alojamento.Pais_Residencia_Origem.value).toBe(bulletin.guest.countryOfResidence);
    expect(Boletim_Alojamento.Tipo_Documento.value).toBe(bulletin.guest.document.type);
});
test("should parse SIBA success response correctly", () => {
    ...
    const sibaResponse = _1.parseSIBAResponse(successResponse);
...
test("should receive success from DEV endpoint", () => __awaiter(this, void 0, void 0, function* () {
    const body = _1.buildSIBARequestXML(bulletin);
    ...
    const sibaREsponse = _1.parseSIBAResponse(xmlResponse);
    expect(sibaREsponse.isSuccess).toBeTruthy();
}), 30000);
